module powerbi.extensibility.visual {
  ...
  export class MapboxMap implements IVisual {
    ...
    private addMap() {
      ...
      const mapOptions = {
        ...
        transformRequest: (url, resourceType) => {
          if (
            url.slice(0, 25) == "https://api.mapbox.com/v4" ||
    ...
    private validateOptions(options: VisualUpdateOptions) {
      ...
      const roles: any = options.dataViews[0].metadata.columns
        .map((column) => {
          if (column.roles) {
      ...
      if (
        (this.settings.circle.show ||
          this.settings.cluster.show ||
          this.settings.heatmap.show) &&
        !(roles.latitude && roles.longitude)
      ) {
        this.errorDiv.innerHTML = Templates.MissingGeo;
        return false;
      } else if (
        this.settings.choropleth.show &&
        (!roles.location || !roles.color || roles.latitude || roles.longitude)
      ) {
    ...
    private updateCurrentLevel(settings, options) {
      ...
      let temp_sources = options.dataViews[0].matrix.rows.levels[0].sources.filter(
        (s) => temp_indexes.indexOf(s.identityExprs[0]["ref"]) > -1
      );
    ...
    public update(options: VisualUpdateOptions) {
      ...
      let style =
        this.settings.api.style == "custom"
          ? this.settings.api.styleUrl
          : this.settings.api.style;
